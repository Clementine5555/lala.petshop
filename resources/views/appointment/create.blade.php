...
<head>
    ...
    <style>
        /* ... (Style tetap sama, saya ringkas biar tidak kepanjangan) ... */
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: url('https://images.unsplash.com/photo-1601758228041-f3b2795255f1?w=1920') center/cover no-repeat fixed; min-height: 100vh; display: flex; align-items: center; justify-content: center; padding: 20px; padding-top: 70px; }
        body::before { content: ''; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(255, 245, 235, 0.85); z-index: -1; }
        ...
        .container { background: rgba(255, 255, 255, 0.95); border-radius: 30px; padding: 50px; max-width: 600px; width: 100%; margin-top: 20px; box-shadow: 0 20px 60px rgba(0,0,0,0.15); }
        .form-title { text-align: center; margin-bottom: 40px; }
        .form-title h2 { font-size: 2em; color: #FF8C42; margin-bottom: 10px; }
        ...
        .form-group input[type="text"], .form-group input[type="email"], .form-group input[type="tel"], .form-group input[type="number"], .form-group input[type="date"], .form-group input[type="time"], .form-group textarea, .form-group select { width: 100%; padding: 12px 15px; border: 2px solid #e0e0e0; border-radius: 10px; font-size: 1em; transition: all 0.3s; }
        .form-group input:focus, .form-group textarea:focus, .form-group select:focus { outline: none; border-color: #FF8C42; }
        .form-group textarea { resize: vertical; min-height: 80px; }
        .radio-group { display: flex; gap: 20px; flex-wrap: wrap; }
        .radio-option { display: flex; align-items: center; gap: 8px; }
        .radio-option input[type="radio"] { width: 20px; height: 20px; accent-color: #FF8C42; cursor: pointer; }
        .radio-option label { cursor: pointer; margin-bottom: 0 !important; }
        .date-time-row { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; }
        .service-checkbox { display: flex; align-items: flex-start; gap: 10px; padding: 15px; border: 2px solid #e0e0e0; border-radius: 10px; margin-bottom: 10px; cursor: pointer; transition: all 0.3s; }
        .service-checkbox:hover { border-color: #FF8C42; background: #FFF5EB; }
        .service-checkbox input[type="checkbox"] { width: 20px; height: 20px; accent-color: #FF8C42; margin-top: 2px; cursor: pointer; }
        .service-info { flex: 1; }
        .service-info strong { display: block; color: #333; margin-bottom: 5px; }
        .service-info span { color: #666; font-size: 0.9em; }
        .link-text { color: #FF8C42; text-decoration: underline; cursor: pointer; font-size: 0.9em; }
        .summary-row { display: flex; justify-content: space-between; padding: 12px 0; border-bottom: 1px solid #e0e0e0; }
        .summary-row:last-child { border-bottom: none; font-weight: 700; font-size: 1.1em; color: #FF8C42; }
        .summary-row label { color: #666; }
        .summary-row span { color: #333; font-weight: 600; }
        .upload-section { margin-top: 20px; }
        .upload-btn { display: inline-block; padding: 12px 30px; background: #FF8C42; color: white; border-radius: 10px; cursor: pointer; transition: all 0.3s; }
        .upload-btn:hover { background: #FF6B35; }
        .upload-btn input[type="file"] { display: none; }
        .file-name { display: inline-block; margin-left: 10px; color: #666; font-size: 0.9em; }
        .button-group { display: flex; gap: 15px; margin-top: 30px; }
        .btn { flex: 1; padding: 15px; border: none; border-radius: 50px; font-size: 1.1em; font-weight: 700; cursor: pointer; transition: all 0.3s; }
        .btn-cancel { background: #e0e0e0; color: #666; }
        .btn-cancel:hover { background: #d0d0d0; }
        .btn-next, .btn-submit { background: linear-gradient(135deg, #FF8C42 0%, #FF6B35 100%); color: white; box-shadow: 0 5px 15px rgba(255, 140, 66, 0.3); }
        .btn-next:hover, .btn-submit:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(255, 140, 66, 0.4); }
        .btn-back { background: white; color: #FF8C42; border: 2px solid #FF8C42; }
        .btn-back:hover { background: #FFF5EB; }
        .alert-info { background: #FFF5EB; padding: 15px; border-radius: 10px; margin-top: 20px; color: #FF8C42; font-weight: 600; text-align: center; }
        ...
        @media (max-width: 768px) { .nav-container { padding: 0 20px; } .logo span { font-size: 1.2em; } .nav-links { display: none; } .container { padding: 30px 20px; } .date-time-row { grid-template-columns: 1fr; } .button-group { flex-direction: column; } }
    </style>
</head>
<body>
...
<div class="modal" id="editProfileModal">
    <div class="modal-content">
        <div class="modal-header"><h2>Edit Profile</h2><svg class="close-modal" onclick="closeEditProfile()" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></div>
...
<div class="container">
    <form action="{{ route('appointment.store') }}" method="POST" enctype="multipart/form-data" id="appointmentForm">
        @csrf

        <div class="form-step active" data-step="1">
            <div class="form-title"><h2>Pet Information</h2></div>
            ...
            <div class="form-group">
                ...
                <div class="radio-group">
                    <div class="radio-option"><input type="radio" id="dog" name="pet_type" value="dog" required><label for="dog">Dog</label></div>
                    <div class="radio-option"><input type="radio" id="cat" name="pet_type" value="cat"><label for="cat">Cat</label></div>
                </div>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <div class="radio-group">
                    <div class="radio-option"><input type="radio" id="female" name="pet_gender" value="female" required><label for="female">Female</label></div>
                    <div class="radio-option"><input type="radio" id="male" name="pet_gender" value="male"><label for="male">Male</label></div>
                </div>
            </div>
            <div class="form-group">
        ...
        <div class="form-step" data-step="2">
            <div class="form-title"><h2>Appointment Details</h2></div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <div class="date-time-row"><input type="time" id="appointment_time" name="appointment_time" required></div>
            </div>
            <div class="form-group">
                ...
                <div style="margin-bottom:12px;">
                    ...
                    <select id="groomer_id" name="groomer_id" required @if(!isset($groomers) || $groomers->count() == 0) disabled @endif>
                        ...
                        <option value="{{ $gr->user_id }}">{{ $gr->name }}</option>
                        @endforeach
                        @endif
                    </select>
                    @if(!isset($groomers) || $groomers->count() == 0)
                ...
                <div class="service-radio-group">
                    ...
                    <label class="service-option">
                        <input type="radio" name="service_id" value="{{ $srv->service_id }}" data-name="{{ $srv->service_name }}" data-price="{{ $srv->price }}" onchange="updateSummary()" {{ (isset($selectedService) && $selectedService->service_id == $srv->service_id) ? 'checked' : '' }} required>
        ...
        <div class="form-step" data-step="3">
            <div class="form-title"><h2>Payment Summary</h2></div>
            <div class="form-group">
                <div class="summary-row"><label>Customer Name:</label><span id="summary_customer">{{ Auth::user()->name ?? 'Guest' }}</span></div>
                <div class="summary-row"><label>Pet Name:</label><span id="summary_pet">-</span></div>
                <div class="summary-row"><label>Appointment Date:</label><span id="summary_date">-</span></div>
                <div class="summary-row"><label>Appointment Time:</label><span id="summary_time">-</span></div>
                <div class="summary-row"><label>Selected Service:</label><span id="summary_service">-</span></div>
                <div class="summary-row"><label>Total Price:</label><span id="summary_price">Rp 0</span></div>
            </div>
            <div class="form-group">
                <label>Payment Method</label>
                <div class="radio-group">
                    <div class="radio-option"><input type="radio" id="cash" name="payment_method" value="cash" required onchange="togglePaymentInfo()"><label for="cash">Cash</label></div>
                    <div class="radio-option"><input type="radio" id="bank_transfer" name="payment_method" value="bank_transfer" onchange="togglePaymentInfo()"><label for="bank_transfer">Bank Transfer</label></div>
                    <div class="radio-option"><input type="radio" id="ewallet" name="payment_method" value="ewallet" onchange="togglePaymentInfo()"><label for="ewallet">E-Wallet (GoPay)</label></div>
                </div>
            </div>
            ...
            <div class="upload-section" id="upload-section" style="display: none;">
                ...
                <label class="upload-btn">Choose File<input type="file" name="payment_proof" accept="image/*" id="payment_proof" onchange="showFileName()"></label>
...
<script>
    ...
    function nextStep(step) {
        ...
        inputs.forEach(input => {
            if (!input.value.trim()) { valid = false; input.style.border = "2px solid red"; } else { input.style.border = "1px solid #ddd"; }
        });
        const serviceRadios = currentStepDiv.querySelectorAll('input[name="service_id"]');
        if (serviceRadios.length > 0) {
            if (!currentStepDiv.querySelector('input[name="service_id"]:checked')) { valid = false; alert("⚠️ Please select at least one service!"); }
        }
        if (!valid) return;
    ...
    function updateSummary() {
        ...
        if (petInput && petInput.value && petInput.value.trim() !== '') { petName = petInput.value.trim(); }
        ...
        if (selectedService) {
            ...
            const price = parseInt(String(selectedService.dataset.price).replace(/\D/g, '')) || 0;
    ...
    function submitForm() {
        ...
        if (!paymentMethod) { Swal.fire({ icon: 'warning', title: 'Pilih metode pembayaran', confirmButtonColor: '#FF8C42'}); return false; }
    ...
    function showFileName() {
        ...
        if (input.files.length > 0) document.getElementById('file-name').textContent = input.files[0].name;
    }
    ...
    function togglePaymentInfo() {
        ...
        if (method === 'cash') document.getElementById('cash-info').style.display = 'block';
        else if (method === 'bank_transfer') {
            document.getElementById('transfer-info').style.display = 'block';
            document.getElementById('transfer-details').textContent = 'SeaBank - 901683597161';
            document.getElementById('upload-section').style.display = 'block';
        } else if (method === 'ewallet') {
            document.getElementById('transfer-info').style.display = 'block';
            document.getElementById('transfer-details').textContent = 'GoPay - 081260968298';
            document.getElementById('upload-section').style.display = 'block';
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (petSelect && petInput) {
            const handlePetChange = () => {
                if (petSelect.value === 'new' || !petSelect.value) { petInput.style.display = 'block'; petInput.required = true; }
                else { petInput.style.display = 'none'; petInput.required = false; }
            };
            petSelect.addEventListener('change', handlePetChange);
            handlePetChange();
        } else if (petInput) { petInput.style.display = 'block'; petInput.required = true; }
    });

    @if(session('success'))
        Swal.fire({ icon: 'success', title: 'Yeay! Berhasil 🎉', text: '{{ session('success') }}', confirmButtonColor: '#FF8C42' });
    @endif
    @if($errors->any())
        const laravelErrors = {!! json_encode($errors->all()) !!};
    Swal.fire({ icon: 'error', title: 'Waduh!', html: '<div style="text-align:left">' + laravelErrors.map(e => '<div>• ' + e + '</div>').join('') + '</div>', confirmButtonColor: '#d33' });
    @endif
</script>
</body>
</html>
