...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; overflow-x: hidden; padding-top: 50px; background: #f5f5f5; }
        ...
        .profile-avatar { width: 34px; height: 34px; border-radius: 50%; object-fit: cover; border: 2px solid #FF8C42; flex-shrink: 0; }
        ...
        .dropdown-menu { position: absolute; top: calc(100% + 15px); right: 0; background: white; border-radius: 14px; box-shadow: 0 8px 30px rgba(0,0,0,0.15); min-width: 240px; opacity: 0; visibility: hidden; transform: translateY(-10px); transition: all 0.3s; }
        ...
        /* Products Page */
        .products-page { padding: 40px 20px; max-width: 1400px; margin: 0 auto; }
        .page-header { text-align: center; margin-bottom: 50px; }
        .page-header h1 { font-size: 2.8em; color: #333; font-weight: 800; margin-bottom: 15px; }
        .page-header p { font-size: 1.2em; color: #666; }

        .search-section { max-width: 600px; margin: 0 auto 40px; position: relative; }
        .search-box { width: 100%; padding: 18px 25px 18px 55px; border: 2px solid #e0e0e0; border-radius: 50px; font-size: 1.1em; transition: all 0.3s; background: white; }
        .search-box:focus { outline: none; border-color: #FF8C42; box-shadow: 0 5px 20px rgba(255, 140, 66, 0.2); }
        .search-icon { position: absolute; left: 20px; top: 50%; transform: translateY(-50%); font-size: 1.3em; }

        /* Category Filter (Updated Styles for Links) */
        .category-filter { display: flex; gap: 15px; justify-content: center; flex-wrap: wrap; margin-bottom: 50px; padding: 0 20px; }
        .category-btn { padding: 14px 30px; border-radius: 50px; border: 2px solid #e0e0e0; background: white; color: #666; font-weight: 600; font-size: 1em; cursor: pointer; transition: all 0.3s; display: flex; align-items: center; gap: 10px; text-decoration: none; }
        .category-btn:hover { border-color: #FF8C42; color: #FF8C42; transform: translateY(-3px); box-shadow: 0 5px 15px rgba(255, 140, 66, 0.2); }
        .category-btn.active { background: linear-gradient(135deg, #FF8C42 0%, #FF6B35 100%); color: white; border-color: #FF8C42; box-shadow: 0 5px 20px rgba(255, 140, 66, 0.4); }

        .products-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 30px; margin-bottom: 50px; }
        .product-card { background: white; border-radius: 25px; padding: 25px; transition: all 0.3s; box-shadow: 0 5px 15px rgba(0,0,0,0.08); cursor: pointer; }
        .product-card:hover { transform: translateY(-8px); box-shadow: 0 15px 35px rgba(0,0,0,0.15); }
        .product-image-wrapper { width: 100%; height: 220px; display: flex; align-items: center; justify-content: center; background: #f9f9f9; border-radius: 20px; margin-bottom: 20px; }
        .product-image-wrapper img { max-width: 90%; max-height: 90%; object-fit: contain; }
        .product-info h3 { font-size: 1.4em; color: #333; margin-bottom: 10px; font-weight: 700; min-height: 60px; }
        .product-rating { display: flex; align-items: center; gap: 8px; margin-bottom: 12px; }
        .stars { display: flex; gap: 3px; }
        .star { color: #FFD700; font-size: 1.1em; }
        .star.empty { color: #ddd; }
        .review-count { color: #666; font-size: 0.95em; }
        .product-price { font-size: 1.8em; color: #FF8C42; font-weight: 800; margin-bottom: 15px; }
        .stock-status { display: inline-flex; align-items: center; gap: 6px; padding: 6px 15px; background: #e8f5e9; color: #2e7d32; border-radius: 20px; font-size: 0.9em; font-weight: 600; margin-bottom: 15px; }
        .stock-status.out-of-stock { background: #ffebee; color: #c62828; }

        .product-actions { display: flex; gap: 10px; }
        .btn { flex: 1; padding: 12px 20px; border: none; border-radius: 25px; font-weight: 700; font-size: 1em; cursor: pointer; transition: all 0.3s; }
        .btn-primary { background: linear-gradient(135deg, #FF8C42 0%, #FF6B35 100%); color: white; box-shadow: 0 4px 15px rgba(255, 140, 66, 0.3); }
        .btn-primary:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(255, 140, 66, 0.5); }
        ...
        .product-modal { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.6); z-index: 2000; align-items: center; justify-content: center; padding: 20px; }
        .product-modal.active { display: flex; }
        .product-modal-content { background: white; border-radius: 30px; width: 100%; max-width: 1000px; max-height: 90vh; overflow-y: auto; animation: modalSlideIn 0.3s ease-out; }
        @keyframes modalSlideIn { from { transform: translateY(-50px); opacity: 0; } to { transform: translateY(0); opacity: 1; } }
        .product-modal-header { padding: 30px 40px 20px; border-bottom: 2px solid #f0f0f0; position: relative; }
        .product-modal-close { position: absolute; right: 25px; top: 25px; width: 40px; height: 40px; cursor: pointer; color: #999; transition: all 0.3s; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.5em; }
        .product-modal-close:hover { background: #f5f5f5; color: #333; }
        .product-modal-body { padding: 30px 40px; display: grid; grid-template-columns: 1fr 1fr; gap: 40px; }
        .product-modal-image { display: flex; align-items: center; justify-content: center; background: #f9f9f9; border-radius: 20px; padding: 30px; min-height: 400px; }
        .product-modal-image img { max-width: 100%; max-height: 400px; }
        .product-modal-details h2 { font-size: 2em; color: #333; margin-bottom: 15px; }
        .product-modal-description { color: #666; line-height: 1.8; margin-bottom: 20px; }
        .quantity-selector { display: flex; align-items: center; gap: 15px; margin: 20px 0; }
        .quantity-controls { display: flex; align-items: center; background: #f0f0f0; border-radius: 30px; }
        .quantity-btn { width: 40px; height: 40px; border: none; background: transparent; font-size: 1.3em; cursor: pointer; transition: all 0.3s; }
        .quantity-btn:hover { background: #e0e0e0; }
        .quantity-input { width: 50px; text-align: center; border: none; background: transparent; font-size: 1.1em; font-weight: 600; }

        /* Reviews Section */
        .reviews-section { padding: 30px 40px; background: #f9f9f9; border-top: 2px solid #e0e0e0; }
        .reviews-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; }
        .reviews-header h3 { font-size: 1.6em; color: #333; }
        .btn-add-review { padding: 10px 25px; background: #FF8C42; color: white; border: none; border-radius: 20px; font-weight: 600; cursor: pointer; transition: all 0.3s; }
        .btn-add-review:hover { background: #FF6B35; transform: translateY(-2px); }
        .review-form { display: none; background: white; padding: 25px; border-radius: 20px; margin-bottom: 25px; }
        .review-form.active { display: block; }
        .form-group { margin-bottom: 20px; }
        .form-group label { display: block; color: #333; font-weight: 600; margin-bottom: 8px; }
        .rating-input { display: flex; gap: 8px; font-size: 2em; }
        .rating-input span { cursor: pointer; color: #ddd; transition: all 0.2s; }
        .rating-input span:hover, .rating-input span.active { color: #FFD700; }
        .form-control { width: 100%; padding: 12px 18px; border: 2px solid #e0e0e0; border-radius: 12px; font-size: 1em; font-family: inherit; }
        .form-control:focus { outline: none; border-color: #FF8C42; }
        textarea.form-control { resize: vertical; min-height: 120px; }
        .form-actions { display: flex; gap: 12px; }
        .review-item { background: white; padding: 20px; border-radius: 15px; margin-bottom: 15px; }
        .review-header { display: flex; justify-content: space-between; margin-bottom: 10px; }
        .review-author { display: flex; align-items: center; gap: 12px; }
        .author-avatar { width: 45px; height: 45px; border-radius: 50%; background: linear-gradient(135deg, #FF8C42, #FF6B35); display: flex; align-items: center; justify-content: center; color: white; font-weight: 700; font-size: 1.2em; }
        .author-info h4 { font-size: 1.1em; color: #333; }
        ...
        @media (max-width: 992px) { .nav-links { gap: 20px; } .nav-links a { font-size: 1.1em; } .profile-name { display: none; } .product-modal-body { grid-template-columns: 1fr; } .category-filter { gap: 10px; } .category-btn { padding: 10px 20px; font-size: 0.9em; } }
        @media (max-width: 768px) { .nav-container { padding: 0 20px; } .logo span { font-size: 1.2em; } .nav-links { display: none; } .products-grid { grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); } .category-btn .emoji { font-size: 1.2em; } .category-btn span:not(.emoji) { display: none; } .category-btn { padding: 10px 15px; } }
    </style>
</head>
<body>
...
<div class="products-page">
    ...
    <div class="category-filter">
        ...
        <a href="{{ route('products.shop', ['category' => 'Dog Food']) }}"
           class="category-btn {{ request('category') == 'Dog Food' ? 'active' : '' }}"
           style="text-decoration: none;">
            <span>Makanan Anjing</span>
        </a>

        <a href="{{ route('products.shop', ['category' => 'Cat Food']) }}"
           class="category-btn {{ request('category') == 'Cat Food' ? 'active' : '' }}"
           style="text-decoration: none;">
            <span>Makanan Kucing</span>
        </a>

        <a href="{{ route('products.shop', ['category' => 'Vitamin']) }}"
           class="category-btn {{ request('category') == 'Vitamin' ? 'active' : '' }}"
           style="text-decoration: none;">
            <span>Vitamin & Suplemen</span>
        </a>

        <a href="{{ route('products.shop', ['category' => 'Toys']) }}"
           class="category-btn {{ request('category') == 'Toys' ? 'active' : '' }}"
           style="text-decoration: none;">
            <span>Mainan</span>
        </a>
    </div>
...
<script>
    ...
    // ============================================
    // SEARCH FUNCTIONALITY (Client Side Text Filter)
    // ============================================
    document.getElementById('searchInput').addEventListener('input', function(e) {
        ...
        cards.forEach(card => {
            ...
            // Only check text content (category is handled by server)
            if (title.includes(searchTerm)) {
                card.style.display = 'flex'; // Reset display to flex
                visibleCount++;
            } else {
        ...
        if (visibleCount === 0) {
            if (!noProductsMsg) {
                ...
                noProductsMsg.innerHTML = '<h3>Tidak ada produk dengan nama tersebut.</h3>';
        ...
        fetch(`/api/products/${productId}/review`, {
            ...
            .then(async ({ ok, data }) => {
                if (ok && data && data.success) {
                    ...
                    // Update local product data for modal
                    const product = productsData.find(p => p.product_id === productId);
                    if (product) {
                        const productReviews = allReviews[productId] || [];
